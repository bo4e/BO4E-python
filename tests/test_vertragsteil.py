...
class TestVertragsteil:
    def test_vertragsteil_only_required_attributes(self):
        ...
        """
        vertragsteil = Vertragsteil(vertragsteilbeginn=date(2001, 3, 15), vertragsteilende=date(2007, 11, 27))

        schema = VertragsteilSchema()
        json_string = schema.dumps(vertragsteil, ensure_ascii=False)

        assert "2001-03-15" in json_string
        assert "2007-11-27" in json_string

        vertragsteil_deserialized = schema.loads(json_string)

        assert isinstance(vertragsteil_deserialized.vertragsteilbeginn, date)
        assert vertragsteil_deserialized.vertragsteilbeginn == date(2001, 3, 15)
        assert isinstance(vertragsteil_deserialized.vertragsteilende, date)
        assert vertragsteil_deserialized.vertragsteilende == date(2007, 11, 27)
    ...
    def test_vertragsteil_required_and_optional_attributes(self):
        ...
        vertragsteil = Vertragsteil(
            vertragsteilbeginn=date(2001, 3, 15),
            vertragsteilende=date(2007, 11, 27),
            lokation="Bar",
    ...
    def test_vertragsteil_missing_required_attribute(self):
        with pytest.raises(TypeError) as excinfo:
            _ = Vertragsteil(vertragsteilende=date(2007, 11, 27))
