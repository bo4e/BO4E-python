...
class TestZeitraum:
    ...
    def test_zeitraum_daten(self):
        ...
        """
        zeitraum = Zeitraum(startdatum=date(2013, 5, 1), enddatum=date(2022, 1, 28))
        ...
        assert isinstance(zeitraum_deserialized.startdatum, date)
        assert zeitraum_deserialized.startdatum == date(2013, 5, 1)
        assert isinstance(zeitraum_deserialized.enddatum, date)
        assert zeitraum_deserialized.enddatum == date(2022, 1, 28)
    ...
    def test_zeitraum_zeitpunkte(self):
        ...
        """
        zeitraum = Zeitraum(startzeitpunkt=datetime(2011, 2, 5, 16, 43), endzeitpunkt=datetime(2021, 7, 30))

        schema = ZeitraumSchema()
        json_string = schema.dumps(zeitraum, ensure_ascii=False)

        assert "2011-02-05T16:43:00" in json_string
        assert "2021-07-30T00:00:00" in json_string

        zeitraum_deserialized = schema.loads(json_string)

        assert isinstance(zeitraum_deserialized.startzeitpunkt, datetime)
        assert zeitraum_deserialized.startzeitpunkt == datetime(2011, 2, 5, 16, 43)
        assert isinstance(zeitraum_deserialized.endzeitpunkt, datetime)
        assert zeitraum_deserialized.endzeitpunkt == datetime(2021, 7, 30)
